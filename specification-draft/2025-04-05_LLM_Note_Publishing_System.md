...
-   **処理**:
    ...
    -   note.com 非公式APIを呼び出すためのHTTPリクエストを送信する。
        -   APIエンドポイント、リクエストヘッダー（認証情報含む）、リクエストボディ（変換後Markdown）を正しく設定する。
        -   レスポンスを解析し、下書きURLなどを取得する。
        -   エラーハンドリングを実装する。
-   **出力**: 処理結果（ログ、下書きURL）。

#### 3.2.3 フォーマット指示ファイル (`Docs/templates/note_format_prompt.md`)
-   note記事として適切な構成、文体、見出しレベル、コードブロックの扱い、画像の扱い（もしあれば）などを指示するMarkdown形式のプロンプト。
-   例: 「以下の#調査結果 を元に、日本の読者向けの技術ブログ記事を作成してください。構成は『はじめに』『調査内容の要約』『詳細』『考察』『まとめ』とし、親しみやすい文体で記述してください。コードブロックは適切にフォーマットしてください。」
-   このファイルは別途作成・管理する。
...
-   **要調査項目**:
    -   下書き投稿用エンドポイントURL
    -   必要なHTTPメソッド (POSTなど)
    -   リクエストヘッダー（`Authorization` トークンなど）
    -   リクエストボディの形式（JSONなど。Markdownコンテンツをどのように含めるか）
    -   認証トークンの取得方法
-   **注意**: 非公式APIは予告なく変更・停止される可能性があるため、定期的な動作確認と、変更への対応が必要。利用規約も確認すること。
