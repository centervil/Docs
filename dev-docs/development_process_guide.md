...
### 2.3 テスト駆動開発（TDD）の標準手順

#### テストファイル構成

プロジェクトでは以下のテストディレクトリ構造を標準として採用します：

```
tests/
├── conftest.py          # 共通フィクスチャと設定
├── unit/                # ユニットテスト
│   ├── test_module1.py  # 各モジュールごとのテスト
│   └── test_module2.py
├── integration/         # 統合テスト
...
1. **ユニットテスト**
   ...
   - 例: `markdown_utils.py` モジュールのテストなら `tests/unit/test_markdown_utils.py`
   - 目的: 個々の関数やクラスの機能を分離してテスト
...
1. **テストファイルの作成**
   - 実装予定のモジュールに対応するテストファイルを作成
   - テストケースを設計（正常系、異常系、エッジケース）
   - モック、スタブ、フィクスチャを必要に応じて準備

2. **失敗するテストの実装**
   - まだ実装されていない機能に対するテストを書く
   - `docker-compose run --rm test-unit` でテストが失敗することを確認

3. **最小限の実装**
   - テストを通過させるための最小限のコードを実装
   - パフォーマンスや最適化よりも、まずはテストの通過を優先

4. **テスト通過の確認**
   - `docker-compose run --rm test-unit` でテストが通過することを確認
   - すべてのテストが通過するまで実装を調整

5. **リファクタリング**
   - コードの品質、可読性、効率性を改善
   - テストが引き続き通過することを確認
   - `docker-compose run --rm test-coverage` でカバレッジを確認

6. **統合テストの追加**
   - 機能が他のコンポーネントと連携する場合は統合テストを追加
   - `docker-compose run --rm test` ですべてのテストが通過することを確認

この一連のサイクルを繰り返すことで、品質の高いコードを継続的に開発していきます。
