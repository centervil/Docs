...
1. **最初のステップとして**、今日の日付（YYYY-MM-DD形式）を取得し、開発日記ファイル「Docs/dev-records/YYYY-MM-DD-development.md」を作成してください。
...
## 前日までの活動履歴
Docs/dev-recordsディレクトリ内の日付を確認し、関連する最近の活動を箇条書きで記載してください。
例：
```
前日までの活動履歴は以下の通りです：
* 2025-03-22: Diary-Converterのリポジトリの整理
* 2025-03-21: diary-converterを独立したリポジトリとして移行
...
必要に応じて、会話ログを元にZenn向けの記事を作成します。Zenn記事のテンプレートと詳細な記述ルールは「Docs/dev-docs/zenn_template.md」に定義されています。このファイルを参照してください。
...
- **Docs/dev-docs/zenn_template.md**: Zenn記事のテンプレートと詳細な記述ルール
...
例：
- 2025-04-01-001-diary-converter-simplification-Diary-Converter.md
- 2025-04-02-002-new-feature-MyProject.md

**各要素の説明:**
- **YYYY-MM-DD**: 年月日 (例: 2025-04-01)
- **[通し番号]**: その日の開発記録の連番 (例: 001, 002)
- **[テーマ名]**: 開発テーマを簡潔に英数字とハイフンで表現 (例: diary-converter-simplification)
- **[プロジェクト名]**: 関連するプロジェクト名を英数字で表現 (例: Diary-Converter, MyProject)

### 理由

この命名規則を採用する理由は以下の通りです：

1. **情報の明確化**: ファイル名から日付、連番、テーマ、プロジェクト名を容易に把握できます。
2. **一貫性の確保**: すべての開発日記ファイルが同じ形式になるため、管理が容易になります。
3. **自動処理の効率化**: ファイル名から必要な情報を抽出し、Zenn記事生成などの自動処理に活用できます。
4. **ソート順の明確化**: 日付と連番により、時系列でのソートが容易になります。
5. **国際化対応**: 英数字とハイフンのみを使用することで、文字化けやエンコーディングの問題を回避できます。

### 既存ファイルの扱い

- 既存のファイルはそのままの名前を維持します。
- 新規作成するファイルはすべてこの命名規則に従います。
- 関連するスクリプト（例: Diary-Converter）もこの命名規則を利用して情報を抽出します。
